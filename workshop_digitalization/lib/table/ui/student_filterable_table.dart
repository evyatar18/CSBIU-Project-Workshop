...
Widget createFilterableTableScaffold3(Stream<List<Student>> students) {
  return FilterableTable(
    ...
    otherFilterables: []..addAll(otherFields.map((field) =>
         createCastingFilterableField(createSelectionFilterable(field))))
    ,shownFields: [otherFields[0].name]//..addAll(otherFields.map((field) => field.name)),
    ,onClick: null,
  );
}
